...
yearterm = int(input("데이터 년도 간격 (<15, 종료:-1) : "))
while True:
    if yearterm < 15:
        break
    elif yearterm == '-1':
...
yearterm2 = int(input("데이터 년도 간격 횟수 (연도간격({})*N<15, 종료:-1) : ".format(yearterm)))
...
while True:
    if yearterm*yearterm2 < 15:
        ...
        yearterm2 = int(input("데이터 년도 간격* 데이터 년도 반복 횟수 (<15, 종료:-1) : "))
...
if not illname == illname2:
    ...
    print("STEP99-1:{:5.3f}s, Total:{:5.3f}s\n".format(step_end - step_start, step_end - total_start))

    print("EXP_STEP99-2.CREATE INDEX person ON T120_{} ... START".format(illname2))
    step_start = time.clock()*1000
    mycursor.execute("CREATE INDEX person ON T120_{}(PERSON_ID)".format(illname2))
    mydb.commit()
    step_end = time.clock()*1000
    print("EXP_STEP99-2.CREATE INDEX person ON T120_{} ... DONE".format(illname2))
    print("STEP99-2:{:5.3f}s, Total:{:5.3f}s\n".format(step_end - step_start, step_end - total_start))
...
result = mycursor.fetchall()
columns = [i[0] for i in mycursor.description]
fp = open("./DATA/EXP:{0}_CTR:{4}/CTR_{0}_{1}_{3}YEAR.csv".format(illname2, yearterm, yearterm * (yearterm2-1),
                                                                                   yearterm * (yearterm2), illname2), 'w')
myFile = csv.writer(fp, lineterminator='\n')
myFile.writerow(columns)
myFile.writerows(result)
...
fp.write("\n")
for count in range(yearterm2):
    mycursor.execute("SELECT COUNT(*) FROM CTR_{0}_{1}_{3}YEAR".format(illname, yearterm, yearterm * (count),
                                                                                   yearterm * (count + 1)))
    table_count = mycursor.fetchall()[0][0]
...
if step_yn == 'Y' or step_yn == 'y':
    ...
    r('OR<-exp(para[,1])')
    r('cbind(OR,para[,c(2,4)])')
    r('capture.output(cbind(OR,para[,c(2,4)]), file = "slm_EXP_{0}_{1}_{3}.txt")'.format(illname, yearterm, yearterm * (yearterm2-1),
                                                                                   yearterm * (yearterm2)))
    r('library(ResourceSelection)')
    r('hoslem.test(Slm$y, Slm$fit)')
    r('capture.output(hoslem.test(Slm$y, Slm$fit), file = "slm_EXP_{0}_{1}_{3}.txt",append=TRUE)'.format(illname, yearterm, yearterm * (yearterm2-1),
                                                                                   yearterm * (yearterm2)))