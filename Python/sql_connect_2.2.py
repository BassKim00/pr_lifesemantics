...
print("CREATE {}FOLDER DIRECTORY".format(illname))
try:
    os.makedirs("./DATA/{}".format(illname))
...
fp = open("./DATA/{0}/EXP_{0}_{1}_{3}YEAR.csv".format(illname, yearterm, yearterm * (yearterm2-1),
...
result = mycursor.fetchall()
columns = [i[0] for i in mycursor.description]
fp = open("./DATA/{0}/CTR_{0}_{1}_{3}YEAR.csv".format(illname, yearterm, yearterm * (yearterm2-1),
                                                                                   yearterm * (yearterm2)), 'w')
myFile = csv.writer(fp, lineterminator='\n')
myFile.writerow(columns)
myFile.writerows(result)
