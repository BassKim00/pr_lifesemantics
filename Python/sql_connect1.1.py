...
EXP_STEP3 = """CREATE TABLE T120_{0}_YEAR_JK AS (
	SELECT
   	 ...
   	 T1.`DFAB_GRD_CD`,
   	 T1.`DFAB_PTN_CD`
	FROM test.JK_UNIF_AN1000 AS T1
...
INNER JOIN test.GJ_UNIF_AN1000 AS T2
ON T1.PERSON_ID = T2.PERSON_ID
...
print("EXP_{0}_{1}_{3}YEAR Export ... DONE".format(illname, yearterm, yearterm * (yearterm2-1),
                                                                                   yearterm * (yearterm2)))
print("EXP EXPORT:{:.6f}s, Total:{:.6f}s\n".format(step_end - step_start, step_end - total_start))
...
print("CTR_{0}_{1}_{3}YEAR Export ... DONE".format(illname, yearterm, yearterm * (yearterm2-1),
                                                                                   yearterm * (yearterm2)))
print("CTR EXPORT:{:.6f}s, Total:{:.6f}s\n".format(step_end - step_start, step_end - total_start))

r('getwd()')
r('setwd("C:/Users/LS-COM-00043/Desktop/data mining")')
...
#n수 반영하는 코드 수정
r('samp <- sample(1:30601,2824, replace=FALSE)')
...
r('csv_6Y')
r('write.csv(csv_6Y,"csv_6Y_deleted.csv")')
